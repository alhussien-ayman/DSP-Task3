class GenericEqualizer {
  ...
  initializeEventListeners() {
    ...
    // Add band button - now opens modal
    ...
    document
      .getElementById("realTimeProcessing")
    ...
    // Spectrogram toggle
    document
      .getElementById("showSpectrograms")
      .addEventListener("change", (e) => {
    ...
    document
      .getElementById("exportSpectrograms")
  ...
  // Update the initializeAddBandModal method to include the slider initialization
  initializeAddBandModal() {
    console.log("🎛️ Initializing add band modal...");

    // Initialize Bootstrap modal
    this.addBandModal = new bootstrap.Modal(
      document.getElementById("addBandModal")
    );
    ...
    document.getElementById("confirmAddBand").addEventListener("click", () => {
      this.confirmAddBand();
    });

    // Gain slider listener
    document.getElementById("bandGain").addEventListener("input", (e) => {
      ...
      if (gainFill) {
        ...
        if (gain > 1.0) {
          gainFill.style.background =
            "linear-gradient(to top, #28a745, #ffc107)";
        } else if (gain < 1.0) {
          gainFill.style.background =
            "linear-gradient(to top, #dc3545, #ffc107)";
        } else {
          gainFill.style.background =
            "linear-gradient(to top, #17a2b8, #28a745)";
        }
      }
    });
  ...
  updateBandInfo() {
    const startFreq =
      parseInt(document.getElementById("startFreq").value) || 100;
    const endFreq = parseInt(document.getElementById("endFreq").value) || 1000;

    // Validate and fix range if needed
    if (startFreq >= endFreq) {
      document.getElementById("endFreq").value = startFreq + 100;
    ...
    document.getElementById("centerFreqValue").textContent = centerFreq;
    document.getElementById("bandType").textContent = bandType;
  }

  // Update the openAddBandModal method to reset the slider
  openAddBandModal() {
    ...
    document.getElementById("bandGain").value = 1.0;
  ...
  confirmAddBand() {
    ...
    if (startFreq < 20 || endFreq > 20000) {
      this.showNotification(
        "Frequency range must be between 20Hz and 20,000Hz",
        "error"
      );
      return;
    }
  ...
  initializeFrequencyRangeSlider() {
    ...
    slider.noUiSlider.on("update", (values, handle) => {
      ...
      document.getElementById(
        "currentRangeDisplay"
      ).textContent = `${startFreq} - ${endFreq} Hz`;
  ...
  addPlayheadDragSupport() {
    ...
    const doDrag = (event) => {
      ...
      const constrainedTime = Math.max(
        0,
        Math.min(newTime, this.totalDuration)
      );
  ...
  updatePlayheadPosition(time) {
    if (!this.inputPlayhead || !this.outputPlayhead || !this.totalDuration)
      return;
    ...
    this.bands.forEach((band, index) => {
      ...
      sliderElement.innerHTML = `
                <div class="slider-header">
                    ...
                    <button class="btn btn-sm btn-outline-danger remove-band" data-id="${
                      band.id
                    }">
                ...
                <div class="slider-info">
                    <small class="text-muted">${band.startFreq} - ${
        band.endFreq
      } Hz</small>
                </div>
...
initializeFrequencyChart() {
  ...
  Plotly.newPlot(
    ...
    [
      {
        ...
        name: "Input Spectrum",
        hovertemplate:
          "Frequency: %{x:.0f} Hz<br>Magnitude: %{y:.6f}<extra></extra>", // Updated
      },
      {
        x: [0],
        y: [0],
    ...
    ],
    {
      ...
      yaxis: {
        title: { text: "Magnitude", font: { color: "#6c757d" } }, // Changed from "Magnitude (dB)" to "Magnitude"
        gridcolor: "rgba(128,128,128,0.2)",
        linecolor: "rgba(128,128,128,0.5)",
        showgrid: true,
      },
  ...
  // REMOVE the dB conversion - use the magnitude values directly
  const inputLinear = inputMagnitude; // Use magnitude directly
  const equalizedLinear = equalizedMagnitude; // Use magnitude directly
    ...
    [
      {
        ...
        name: "Input Spectrum",
        hovertemplate:
          "Frequency: %{x:.0f} Hz<br>Magnitude: %{y:.6f}<extra></extra>", // Changed to show more decimal places
      },
      {
        x: frequencies,
        y: equalizedLinear,
  ...
  async handleFileUpload(file) {
    ...
    if (!allowedExtensions.includes(fileExt)) {
      this.showNotification(
        "Unsupported file type. Please use WAV, FLAC, MP3, M4A, AAC, OGG, or AIFF.",
        "error"
      );
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = async (e) => {
        try {
          ...
          this.inputAudioBuffer = await this.audioContext.decodeAudioData(
            e.target.result
          );
          ...
          console.log(
            `✅ Audio data extracted: ${this.audioData.length} samples, ${
              this.sampleRate
            }Hz, ${this.totalDuration.toFixed(2)}s`
          );
          resolve();
        } catch (error) {
          reject(error);
  ...
  updateWaveformPlots() {
    ...
    console.log(
      `✅ Waveform plots updated - Displaying ${displayDuration.toFixed(
        2
      )} seconds`
    );
  }
    ...
    try {
      ...
      try {
        ...
        if (this.audioData && this.sampleRate) {
          const clientSideResult = this.computeSpectrogramClientSide(
            this.audioData,
            this.sampleRate
          );
          ...
          if (this.processedAudioData) {
            const outputClientSideResult = this.computeSpectrogramClientSide(
          ...
          throw new Error(
            "Audio data not available for client-side computation"
          );
        }
      } catch (fallbackError) {
        console.error("❌ Client-side fallback also failed:", fallbackError);
        this.showNotification(
          "Failed to compute spectrograms. Please try another file.",
          "error"
        );
      }
    }
  ...
  computeSpectrogramClientSide(audioData, sampleRate) {
    ...
    try {
      ...
      for (
        let start = 0;
        start + windowSize <= audioData.length;
        start += hopSize
      ) {
  ...
  async generateTestSignal() {
    ...
    try {
      ...
      this.showNotification(
        "Error generating test signal. Please check backend connection.",
        "error"
      );
    }
  }
  ...
  async extractProcessedAudioData(blob) {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = async (e) => {
        try {
          this.outputAudioBuffer = await this.audioContext.decodeAudioData(
            e.target.result
          );
          ...
          console.log(
            `✅ Processed audio data extracted: ${this.processedAudioData.length} samples`
          );
          resolve();
        } catch (error) {
          reject(error);
  ...
  updateOutputWaveform() {
    ...
    console.log(
      `✅ Output waveform updated - Displaying ${displayDuration.toFixed(
        2
      )} seconds`
    );
  }
  ...
  playOutput() {
    if (!this.outputAudioBuffer) {
      this.showNotification(
        "No processed audio available. Please process audio first.",
        "error"
      );
      return;
    }
  ...
  exportAudio() {
    if (this.processedAudioUrl) {
      ...
      this.showNotification(
        "No processed audio available for export.",
        "error"
      );
    }
  }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (preset.bands && Array.isArray(preset.bands)) {
          ...
          console.log(
            `✅ Preset loaded: ${presetName} with ${preset.bands.length} bands`
          );
        } else {
  ...
  async updatePreset() {
    ...
    try {
      ...
      this.showNotification(
        `Failed to update preset: ${error.message}`,
        "error"
      );
    }
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (typeof Plotly === "undefined") {
    ...
    alert(
      "Error: Plotly.js library is required but not loaded. Please check your internet connection and refresh the page."
    );
    return;
  }
